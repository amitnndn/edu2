...
<?php 
	...
	$n = sizeof($id);
	for($i=0;$i<$n;$i++)	{
		$sql = mysql_query("select * from batch_students where course_id=$courseid and student_id=$id[$i];");
		...
		if($j == 1)	{
			...
			while($info1 = mysql_fetch_array($sql1))	{
				...
				while($info2 = mysql_fetch_array($sql2))	{
					echo $info1['first_name']." is already in the ".$info2['name']." batch.<br>"; 
				}
			}
		}
		else	{
			$sql1 = mysql_query("select * from batch where course_id=$courseid;");
			while($info1 = mysql_fetch_array($sql1))	{
